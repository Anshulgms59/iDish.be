...
function foogra_ocdi_import_files() {
    $demos = array();
    $demos[] = array(
        ...
        'import_notice'                => esc_html__( 'Import process may take 5-10 minutes. If you facing any issues please contact our support.', 'foogra' ),
        'preview_url'                  => 'https://demoapus2.com/foogra/',
    );
    return apply_filters( 'foogra_ocdi_files_args', $demos );
}
add_filter( 'pt-ocdi/import_files', 'foogra_ocdi_import_files' );

function foogra_ocdi_after_import_setup( $selected_import ) {
    // Assign menus to their locations.
    $main_menu       = get_term_by( 'name', 'Primary Menu', 'nav_menu' );
    $user_menu   = get_term_by( 'name', 'User Dashboard', 'nav_menu' );
    ...
    if ( file_exists($file) ) {
        foogra_ocdi_import_settings($file);
    }
...
add_action( 'pt-ocdi/after_import', 'foogra_ocdi_after_import_setup' );

function foogra_ocdi_import_settings($file) {
    ...
    if ( !empty($datas['page_options']) ) {
        foogra_ocdi_import_page_options($datas['page_options']);
    }
    if ( !empty($datas['metadata']) ) {
        foogra_ocdi_import_some_metadatas($datas['metadata']);
    }
}

function foogra_ocdi_import_page_options($datas) {
    if ( $datas ) {
...
function foogra_ocdi_import_some_metadatas($datas) {
    if ( $datas ) {
...
function foogra_ocdi_before_widgets_import() {
...
add_action( 'pt-ocdi/before_widgets_import', 'foogra_ocdi_before_widgets_import' );