...
if ( !class_exists("Foogra_Mobile_Menu") ) {
    class Foogra_Mobile_Menu extends Walker_Nav_Menu {
        ...
        public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            ...
            if ( isset($item->apus_text_label) && !empty($item->apus_text_label) ) {
                switch ( $item->apus_text_label ) {
                    case 'label_new':
                        $text_label  = esc_html__('New', 'foogra');
                        break;

                    case 'label_hot':
                        $text_label  = esc_html__('Hot', 'foogra');
                        break;

                    case 'label_featured':
                        $text_label  = esc_html__('Featured', 'foogra');
                        break;
        ...
        public function generateSubMegaMenu( $item, $apus_mega_profile ) {
            if ( $apus_mega_profile ) {
                ...
                if ( $posts && isset($posts[0]) ) {
                    $post = $posts[0];
                    $content = apply_filters( 'foogra_generate_post_builder', do_shortcode( $post->post_content ), $post, $post->ID);
