jQuery(document).ready(function($){
    ...
    // Listings|Agents|Agency Template
    ...
    function show_hide_listing_setting(val) {
        if ( val == 'page-listings.php' ) {
            $('#apus_page_listings_setting').show();
            ...
            if ( !$('#apus_page_listings_show_filter_top').val() || $('#apus_page_listings_show_filter_top').val() == 'yes' ) {
                $('.cmb2-id-apus-page-listings-filter-top-sidebar').show();
            } else {
                $('.cmb2-id-apus-page-listings-filter-top-sidebar').hide();
            }
            ...
            $('#apus_page_listings_show_filter_top').on('change', function(){
            ...
            $('#apus_page_listings_setting').hide();
    ...
    // loading admin icon
    $("body").on("wp_listings_directory_all_loading_icons", function(){
        foogra_get_awesome_icons();
        foogra_get_food_icons();

        if ( $('#apus_tax_icon_font').length ) {
            $('#apus_tax_icon_font').fontIconPicker({
                theme: 'fip-bootstrap',
                source: wp_listings_directory_all_loaded_icons
            });
        }
    });

    function foogra_get_awesome_icons() {
        ...
        wp_listings_directory_all_loaded_icons.fontwesome = icons_load_call;
    }

    function foogra_get_food_icons() {
        var files = document.querySelectorAll('*[id^="foogra-icons"]'), html = '', css;
        ...
        for( var i=0; i < files.length; i++ ){
            css = Array.prototype.map.call( files[i].sheet.cssRules, foogra_css_text ).join('\n');
            css = css.split('::before');
            css.forEach(function( i ){
                i = i.split('.')[1];
                if( i !== undefined && i.indexOf('/') == -1 ) {
                    var class_name = i.replace(/[^a-z-0-9_]/g, "");
                    if ( class_name !== '' ) {
        ...
        wp_listings_directory_all_loaded_icons.foogra = icons_load_call;
    }

    function foogra_css_text(x) {
        return x.cssText;
    }
});