...
<div class="apus-woocommerce-product-gallery-wrapper <?php echo esc_attr(($attachment_ids && has_post_thumbnail())?'':'full-width'); ?>">
	...
	<div class="slick-carousel apus-woocommerce-product-gallery" data-carousel="slick" data-items="1" data-smallmedium="1" data-extrasmall="1" data-pagination="false" data-nav="false" data-slickparent="true">
		...
		if ( has_post_thumbnail() ) {
			$html  = foogra_wc_get_gallery_image_html( $post_thumbnail_id, true );
		} else {
		...
		if ( $attachment_ids ) {
			foreach ( $attachment_ids as $attachment_id ) {
		 		$html  = foogra_wc_get_gallery_image_html( $attachment_id, true );
	...
	<div class="wrapper-thumbs <?php echo esc_attr($count_thumbs <= $nb_columns ? '' : ''); ?>">
		<div class="slick-carousel apus-woocommerce-product-gallery-thumbs" data-carousel="slick" data-items="<?php echo esc_attr($nb_columns); ?>" data-smallmedium="<?php echo esc_attr($nb_columns); ?>" data-extrasmall="<?php echo esc_attr($nb_columns); ?>" data-smallest="<?php echo esc_attr($nb_columns); ?>" data-pagination="false" data-nav="true" data-asnavfor=".apus-woocommerce-product-gallery" data-slidestoscroll="1" data-focusonselect="true" data-centermode="true">
			...
			if ( has_post_thumbnail() ) {
				...
				$html .= foogra_get_attachment_thumbnail( $post_thumbnail_id, 'woocommerce_gallery_thumbnail' );
				$html .= '</div></div>';
			} else {
			...
			foreach ( $attachment_ids as $attachment_id ) {
				$html  = '<div class="woocommerce-product-gallery__image"><div class="thumbs-inner">';
				$html .= foogra_get_attachment_thumbnail( $attachment_id, 'woocommerce_gallery_thumbnail' );
		 		$html .= '</div></div>';
