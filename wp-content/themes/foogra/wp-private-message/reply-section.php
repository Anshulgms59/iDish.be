...
?>
  <div class="recipient-info clearfix">
    ...
    <a href="javascript:void(0);" class="delete-message-btn float-end" data-id="<?php echo esc_attr($selected_post->ID); ?>" data-nonce="<?php echo esc_attr(wp_create_nonce( 'wp-private-message-delete-message-nonce' )); ?>"><?php esc_html_e('Delete Conversation', 'foogra'); ?></a>
  </div>
    ...
    <div class="list-replies-inner">
      ...
      if ( $next_page <= $reply_messages->max_num_pages ) { ?>
        <div class="loadmore-action">
          <a href="javascript:void(0);" class="loadmore-replied-btn" data-paged="<?php echo esc_attr($next_page); ?>" data-parent="<?php echo esc_attr($selected_post->ID); ?>"><?php esc_html_e( 'Load more', 'foogra' ); ?></a>
        </div>
      <?php } ?>
