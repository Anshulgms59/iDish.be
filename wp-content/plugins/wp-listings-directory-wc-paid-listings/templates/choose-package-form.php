...
<form method="post" id="listing_package_selection">
	<?php if ( empty($listing_id) || WP_Listings_Directory_User::is_user_can_edit_listing( $listing_id ) ) { ?>
		<div class="listing_listing_packages_title">
			...
			<h2><?php esc_html_e( 'Choose a package', 'wp-listings-directory-wc-paid-listings' ); ?></h2>
		</div>
	...
	<?php } else { ?>
		<div class="text-warning">
			<?php esc_html_e('Sorry, you can\'t post a listing.', 'wp-listings-directory-wc-paid-listings'); ?>
		</div>
	<?php } ?>
</form>